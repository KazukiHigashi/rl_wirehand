...
class HerSynergyReplayBuffer(HerReplayBuffer):
    ...
    def _compute_synergy_reconstruction_error(self, batch_indices, env_indices):
        # compute PCA-based reconstruction error for real samples

        infos = [{} for _ in range(len(batch_indices))]

        pos_array = self.actions[batch_indices, env_indices]
    ...
    def _get_real_samples(
        ...
        infos = self._compute_synergy_reconstruction_error(batch_indices,env_indices)
